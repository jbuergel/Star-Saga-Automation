...
class TileRecognizer():
    ...
    def recognize(self, tile):
        ...
        return (self.trained_data[tile_hash] if
                tile_hash in self.trained_data else None)
    ...
    def record_training(self, data):
        if self.missing_tiles:
            ...
            self.missing_tiles = [tile for
                                  tile in self.missing_tiles if
                                  self.compute_hash_from_file(tile) not in
                                  self.trained_data]
